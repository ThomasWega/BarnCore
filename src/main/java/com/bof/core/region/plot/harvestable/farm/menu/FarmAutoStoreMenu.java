...
public class FarmAutoStoreMenu extends ChestGui {
    ...
    private void initialize() {
        ...
        this.addPane(new GoBackPane(4, 3, new FarmSettingsMainMenu(this.region)));
    ...
    private Consumer<InventoryClickEvent> handleSelectAction() {
        return event -> new FarmAutoStoreSetMenu(region, null).show(event.getWhoClicked());
    }
    ...
    private Consumer<InventoryClickEvent> handleSelectAction(FarmPlot plot) {
        return event -> {
            if (event.getClick() == ClickType.SHIFT_LEFT || event.getClick() == ClickType.SHIFT_RIGHT) {
                ...
                // this.update()
                new FarmAutoStoreMenu(this.region).show(event.getWhoClicked());
                return;
            }
            // if not click shift, open the select menu
            new FarmAutoStoreSetMenu(region, plot).show(event.getWhoClicked());
        };
    }
}
