...
public class AnimalsAutoStoreMenu extends ChestGui {
    ...
    public AnimalsAutoStoreMenu(@NotNull BarnRegion region) {
    ...
    private Consumer<InventoryClickEvent> handleSelectAction() {
        return event -> new AnimalsAutoStoreSetMenu(region, null).show(event.getWhoClicked());
    }
    ...
    private Consumer<InventoryClickEvent> handleSelectAction(AnimalPlot plot) {
        return event -> {
            if (event.getClick() == ClickType.SHIFT_LEFT || event.getClick() == ClickType.SHIFT_RIGHT) {
                ...
                // this.update()
                new AnimalsAutoStoreMenu(this.region).show(event.getWhoClicked());
                return;
            }
            // if not click shift, open the select menu
            new AnimalsAutoStoreSetMenu(region, plot).show(event.getWhoClicked());
        };
    }
}
